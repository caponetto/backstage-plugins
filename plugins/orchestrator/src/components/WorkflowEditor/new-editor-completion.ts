...
export const initCompletion = (
  ...
  return monaco.languages.registerCompletionItemProvider(['json', 'yaml'], {
    ...
    provideCompletionItems: async (
      ...
      const lsCompletionItems = await languageServer.getCompletionItems({
        ...
        cursorWordRange: {
          start: {
            line: cursorPosition.lineNumber - 1,
            character:
              (currentWordPosition?.startColumn ?? cursorPosition.column) - 1,
          },
          end: {
            line: cursorPosition.lineNumber - 1,
            character:
              (currentWordPosition?.endColumn ?? cursorPosition.column) - 1,
          },
        },
      });
      ...
      const monacoCompletionItems: monaco.languages.CompletionItem[] =
        lsCompletionItems.map(c => ({
          ...
          insertTextRules:
            c.insertTextFormat === ls.InsertTextFormat.Snippet
...
function toMonacoCompletionItemKind(
  lsCompletionItemKind: ls.CompletionItemKind | undefined,
) {
  switch (lsCompletionItemKind) {
    ...
    default:
      throw new Error(
        "Can't convert from LS Completion Kind to Monaco Completion Kind",
      );
  }
}
